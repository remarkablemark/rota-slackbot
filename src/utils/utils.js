 ...
 */
exports.getUserID = function getUserID(usermention) {
 ...
 */
exports.rotationInList = function rotationInList(rotaname, list) {
  if (list && list.length) {
 ...
 */
exports.isCmd = function isCmd(cmd, input) {
  const msg = cleanText(input);
  const regex = new RegExp(regex[cmd]);
 ...
 */
exports.parseCmd = function parseCmd(cmd, e, ct) {
  const cleanText = cleanText(e.text);
  // Match text using regex associated with the passed command
  const res = [...cleanText.matchAll(new RegExp(regex[cmd]))][0];
 ...
 */
exports.msgConfig = function msgConfig(botToken, channelID, text) {
  return {
    token: botToken,
    channel: channelID,
    text: text,
 ...
 * @return {object} configuration object
 */
exports.msgConfigBlocks = function msgConfigBlocks(
  botToken,
  channelID,
  blocks
) {
 ...
 * @return {object} configuration object
 */
exports.msgConfigEph = function msgConfigEph(botToken, channelID, user, text) {
  return {
    token: botToken,
    channel: channelID,
    user: user,
 ...
 */
exports.ignoreMention = async function ignoreMention({ message, event, next }) {
